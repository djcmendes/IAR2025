...
VELOCITY_DIFF_PENALTY_FACTOR = 0.2  # Penalty factor for velocity differences
...
class Evolution:
    def __init__(self):
        ...
        self.step_count = 0
        self.time_in_line = 0
        self.velocity_diff_accumulator = 0  # Accumulator for velocity differences
        self.penalty_factor = VELOCITY_DIFF_PENALTY_FACTOR
    ...
    def reset(self):
        ...
        self.step_count = 0
        self.collision = False
        self.velocity_diff_accumulator = 0  # Reset penalty accumulator
    ...
    def run_step(self, genome):
        ...
        elif sensor_left or sensor_right:
            self.time_in_line += 5
        ...
        # Set motor speeds (scaled to MAX_SPEED)
        left_speed = output[0] * MAX_SPEED
        right_speed = output[1] * MAX_SPEED
        
        # Cap speeds to valid range
        left_speed_capped = max(min(left_speed, MAX_SPEED), -MAX_SPEED)
        right_speed_capped = max(min(right_speed, MAX_SPEED), -MAX_SPEED)
        
        # Calculate velocity difference for penalty
        velocity_diff = abs(left_speed_capped - right_speed_capped)
        self.velocity_diff_accumulator += velocity_diff
        
        self.left_motor.setVelocity(left_speed_capped)
        self.right_motor.setVelocity(right_speed_capped)
        ...
        try:
            for gen in range(start_gen, GENERATIONS):
                ...
                for idx, individual in enumerate(self.population):
                    ...
                    # Calculate base fitness (normalized time on line)
                    base_fitness = self.time_in_line / EVALUATION_TIME
                    
                    # Calculate velocity difference penalty (normalized to max possible difference)
                    max_possible_diff = 2 * MAX_SPEED * self.step_count
                    penalty = self.velocity_diff_accumulator / max_possible_diff if max_possible_diff > 0 else 0
                    
                    # Apply penalty to fitness
                    fitness = max(0, base_fitness - self.penalty_factor * penalty)
                    individual['fitness'] = fitness
                    
                    print(f" - Fitness: {fitness:.4f} (Base: {base_fitness:.4f}, Penalty: {penalty:.4f})")
